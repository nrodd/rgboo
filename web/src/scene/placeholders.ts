...
export function makePlaceholder(art: SceneArtwork, options: { floorY?: number } = {}) {
  ...
  if (id === "wall") {
    ...
    g.rect(0, 0, w, h).fill(0x201b21);
    for (let x = 24; x < w; x += 96) {
      g.rect(x, 0, 3, panel).fill(0x292128);
      for (let y = 36; y < panel - 24; y += 80) {
        pixels(g, ["..a..", ".aba.", "ab.ba", ".aba.", "..a.."], { a: 0x352830, b: 0x453137 }, 3, x + 38, y);
      }
    }
    // Walnut panelling and a low dado rail make the stand feel grounded in a room.
    g.rect(0, panel, w, floor - panel).fill(0x332624);
    g.rect(0, panel, w, 5).fill(0x614634).rect(0, panel + 5, w, 5).fill(0x211a1e);
    for (let x = 0; x < w; x += 72) {
      g.rect(x, panel + 10, 3, floor - panel - 10).fill(0x221a1c);
      g.rect(x + 4, panel + 10, 2, floor - panel - 10).fill(0x493127);
    }
    g.rect(0, floor - 6, w, 6).fill(0x6a4932).rect(0, floor, w, h - floor).fill(0x1d181c);
    ...
    g.rect(0, 0, w, h).fill(0x101723);
    g.rect(0, h * 0.55, w, h * 0.45).fill(0x162031);
    ...
    for (const [x, y, bw, bh] of [[-8, -8, w + 16, 24], [-8, h - 16, w + 16, 24], [-8, 0, 24, h], [w - 16, 0, 24, h], [w / 2 - 10, 0, 20, h], [0, h / 2 - 10, w, 20]]) {
      ...
      g.rect(x, y, bw, 4).fill(0x584852);
      g.rect(x, y, 4, bh).fill(0x584852);
    }
    ...
    for (let row = 0; row < h; row += 8) {
      ...
      g.rect(start, row, spread, 8).fill(0x492e36);
      for (let col = 6; col < spread; col += 16) g.rect(start + col, row, 4, 8).fill(0x643f48);
    }
    ...
    g.rect(0, 0, w, h).fill(0x584852).rect(0, 0, w, 6).fill(0x7b626c);
