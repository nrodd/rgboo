...
/**
 * Every colour picked, one square per submission, in arrival order.
 *
 * Nothing is encoded and nothing is reordered, so popularity is not
 * readable here -- the family bar underneath carries that, in numbers.
 */
...
export const ColorMosaic = ({ sequence, colors, total, sampled, isRefreshing }: Props) => (
  <section className="stats-card" aria-labelledby="mosaic-heading">
    ...
    {sequence.length === 0 ? (
      ...
      <>
        {/* No hover layer: every number is in the family bar below or the
            table further down. */}
        <div
...
/** Which colour won, in numbers -- the one thing the mosaic cannot show. */
