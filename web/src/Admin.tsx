...
const ADMIN_API_URL = "https://api-admin.rgboo.com";
...
const Admin = () => {
  ...
  const refresh = useCallback(async () => {
    ...
    try { setStatus(await request(`${ADMIN_API_URL}/admin/status`)); }
  ...
  const checkHealth = async () => {
    setBusy(true); setError("");
    try { setHealth(await request(`${ADMIN_API_URL}/`)); }
  ...
  return <main className="min-h-dvh px-6 py-10 text-bone sm:px-12"><div className="mx-auto flex max-w-4xl flex-col gap-8">
    ...
    {status && <><section className="grid gap-5 sm:grid-cols-2">
      <div className="rounded-md border border-pumpkin-400 p-5"><p className="text-sm uppercase opacity-70">Current user</p><p className="mt-2 text-md font-bold">{status.current_username || "Nobody"}</p><button onClick={() => void action(`${ADMIN_API_URL}/admin/clear-current`, "Clear the current user from the overlay?", "{}")} disabled={busy || !status.current_username} className="form-field form-button mt-5 w-full px-4">Clear current user</button></div>
      <div className="rounded-md border border-pumpkin-600 p-5"><p className="text-sm uppercase opacity-70">Queue</p><p className="mt-2 text-md font-bold">{status.queue_size} pending</p><button onClick={() => void action(`${ADMIN_API_URL}/admin/queue/clear`, "Clear the entire pending queue?")} disabled={busy || status.queue_size === 0} className="form-field form-button mt-5 w-full px-4">Clear entire queue</button></div>
